...
class BaseballGame {
    ...
    func start() {
        ...
        // while true 공부 - (게임을 계속 돌려야 해서 사용)
        while true {
            ...
            // 3. 정답과 유저의 입력값을 비교하여 스트라이크/볼을 출력하기 / 0 strikes, 0 balls 일 때 Nothing 추가
    ...
    func makeAnswer() -> [Int] {
        /* 함수 내부를 구현하기 , 서로 다른 임의의 수 3개 생성하기
         해설영상 : .map (고차함수) 사용해보기 -> 클로저 이해하기 , shuffle(), shuffled() 메서드 이해하기
          var arr = (1...9).map { $0 }   기존 코드를 압축? 하여 간단하게 사용하는 방법.      */
...
/*
 Lv 1 , Lv 2 해설 영상 후 보완 , 추가적으로 공부해야 할 것들
 - 클로저 , 고차함수
 .map         - 입력받은 수를 배열로 담는 메서드
 .compactMap  - 옵셔널을 제거한 뒤 배열로 담아주는 메서드
 .shuffle()   - 배열을 섞어주는 메서드 (랜덤으로 뽑을 때 사용할 수 있음)
 .shuffled()
 
 enumerated() - 코드로 썼던 메서드 , 추가 공부 필요
